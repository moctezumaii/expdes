...
```{r load-biomass-data, output =TRUE}
...
```{r fit-biomass-linear, output =TRUE}
...
```{r diagnostics-biomass-linear, fig.width=8, fig.height=4, output =TRUE}
par(mfrow = c(1, 2))
...
```{r plot-biomass-linear, fig.width=7, fig.height=5, output =TRUE}
...
```{r fit-biomass-quadratic, output =TRUE}
...
```{r diagnostics-biomass-quadratic, fig.width=8, fig.height=4, output =TRUE}
par(mfrow = c(1, 2))
...
```{r plot-biomass-both, fig.width=7, fig.height=5}
...
First I will ask for an AIC table. Setting `second.ord = FALSE` tells `aictab()` to report AIC instead of AICc.

```{r biomass-aic, output =TRUE}
aictab(
...
Now I will run the default corrected table, which reports AICc.

```{r biomass-aicc, output =TRUE}
aictab(
  cand.set = list(linear = biomass_linear, quadratic = biomass_quadratic)
)
```
...
```{r load-seedling-data, output =TRUE}
...
```{r fit-seedling-linear, output =TRUE}
seedling_linear <- glm(
...
```{r plot-seedling-linear, fig.width=7, fig.height=5}
...
```{r diagnostics-seedling-linear, fig.width=7, fig.height=5, output =TRUE}
plot(
...
```{r fit-seedling-polynomials, output =TRUE}
...
```{r diagnostics-seedling-cubic, fig.width=7, fig.height=5, output =TRUE}
plot(
...
Today you saw the same workflow twice:

1. Fit a model that is too simple.
2. Diagnose the residual pattern.
3. Add the polynomial terms the biology suggests.
4. Re-check the diagnostics.
5. Compare candidate models with `AICcmodavg`.

That workflow is often more useful than memorizing one "correct" formula.
