---
title: "Assignment 1 and 2. Intro to R, Projects, and Quarto and Parameters and Statistics"
...
Introduction to R, Projects, Quarto and parameters and statistics
...
### Without using a project (you can skip this section)

For this class you are expected to use a project. You can skip this section
...
You can change your working directory, to the folder where your data is located, and then you don't have to write the flepath. Check this website: <https://dzchilds.github.io/eda-for-bio/working-directories-and-data-files.html>

### Using a project (Do this!)
...
Create a single project for the whole course
...
2.  Inside the Quarto file, using a code chunk, load the dataset, and then remove the first column `X` as we did earlier.
3.  Look at the whole dataset
4.  Write the difference between parameters and statistics (Question 1)
:::

Let's obtain some statistics or estimates:

::: callout-warning
## ✏️ Question 2 ✏️ 5pts

1.  In the same file, create an object with only monarchs, one object with only viceroy, and one with only queen.
2.  Estimate the mean forewing area for each group.
3.  Estimate the median for each species (function: `median`)
4.  Estimate the variance and standard deviation for the whole dataset. `sd` function estimates the standard deviation, and `var` does the variance
5.  Estimate the standard error of the mean for each species
6.  
:::
...
Estimate a confidence interval for two species (you can choose which one.
...
Where $\sigma$ is the standard deviation of the population (using the sample sd to estimate the SE).

For the confidence interval, simply multiply the SE by two... this is a pretty good approximation
:::
