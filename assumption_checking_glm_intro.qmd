...
```{r load-data1, output =TRUE}
# Read the yield dataset
yield_data <- read.csv("yield_transform_dataset.csv")
...
```{r diagnostics-raw, fig.width=8, fig.height=4, output =TRUE}
...
```{r, output =TRUE}
library(ggplot2)
ggplot(yield_data, aes(nitrogen_kg_ha, yield_kgha, col=variety)) +
 geom_point() +
...
```{r, output =TRUE}
library(ggplot2)
ggplot(yield_data, aes(nitrogen_kg_ha, log_yield, col=variety)) +
 geom_point() +
 stat_smooth(method = "lm", formula = y ~ x)+
theme_classic()
```
...
> 1.  Which ANOVA type is most appropriate here, and why? *(Hint: think about the study design: is it balanced? Is the interaction important?)*
>
> 2.  Is the interaction between nitrogen and variety statistically significant? Look at the ANOVA table you chose and report the p-value.
>
...
Should we do a Posthoc?

The groups are not different! But there is an interaction.

Are the slopes different?

```{r, output =TRUE}
library(emmeans)
emtrends(log_model_t3, pairwise ~ variety, var = "nitrogen_kg_ha")
```

“Do varieties differ at 0, 50, and 100 kg/ha nitrogen?”

```{r, output =TRUE}
emm <- emmeans(log_model_t3, ~ variety | nitrogen_kg_ha,
               at = list(nitrogen_kg_ha = c(0, 50, 100)))
...
```{r load-data2, output =TRUE}
# Read the sprouting dataset
sprout_data <- read.csv("seed_sprouting.csv")
...
```{r plot-wrong-model, fig.width=7, fig.height=5, output =TRUE}
...
> 1.  **What range do the fitted values fall in: is that a problem?** *(Look at the prediction plot. Do any predicted "probabilities" go below 0 or above 1?)*
>
> 2.  **What does the residual plot look like — does it meet the assumptions of a linear model?** *(Describe what you see.)*
>
> 3.  **What kind of model should we use instead, and why?**
>
> *(Submit your answers on Canvas — do not write them here.)*
...
```{r fit-logistic}
logistic_model <- glm(sprout ~ seed_size_mm, data = sprout_data, family = binomial(link = "logit"))
...
```{r plot-logistic, fig.width=7, fig.height=5}
# Create a sequence of seed sizes for prediction
# Predict with 95% confidence interval on the link scale
# I will use the same dataframe
preds <- predict.glm(logistic_model, sprout_data, se.fit = TRUE)
# Calculate the confidence intervals on the link scale
ci_lwr <-with(preds, plogis(fit - 1.96 * se.fit))
ci_upr <-with(preds, plogis(fit + 1.96 * se.fit))
# Add the predicted probabilities and confidence intervals to the original data frame
sprout_data<-cbind(sprout_data,preds)
sprout_data$predicted_prob <- plogis(sprout_data$fit)
sprout_data$ci_lwr <- ci_lwr
sprout_data$ci_upr <- ci_upr
```

```{r plot-logistic2, fig.width=7, fig.height=5}
ggplot(sprout_data,aes(x=seed_size_mm,y=predicted_prob,ymin=ci_lwr,ymax=ci_upr))+
  geom_point()+
  geom_line(aes(y=predicted_prob))+
  geom_ribbon(alpha=0.15)+
  xlab("Seed Size (mm)")+
  ylab("Probability of Sprouting")+
  theme_bw()
```
